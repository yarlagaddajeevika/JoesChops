...
def authenticateUser(username,password,flag):
...
def StoreCustomisationDetails(custId, empName, itemName, itemdesp):
   if custId=="":
      ...
      cur.execute(f"""insert into Customization_Detail values({custId},{vIn[0][0]},{empID[0][0]})""")
      cur.commit()
      ...
      cur.execute(f"""insert into Item values({planId[0][0]},{empID[0][0]},'{itemName}','{itemdesp}',400.96,150,100,150)""")
      cur.commit()
...
def StoreQuestions(custId, date, question): #date should be a string
   if custId=="":
      ...
      cur.execute(f"""insert into Questionnaire values({planId},'{date}','{question}',null)""")
      cur.commit()
...
def getEmployeeCustomisations(empId):
   if empId=="":
      ...
      for i in cur:
         result.append(i)
      return result[0] # returns the complete data in array format might be in ()
...
def UpdateDB(empId, photoLink, DeliveryDate):
   ...
   if photoLink=="" and DeliveryDate!="":
      cur.execute(f"""update Customization_Plan set Estimated_DeliveryDate='{DeliveryDate}' where Plan_ID={empPland[0][0]}""" )
      cur.commit()
      print("Successfully updated delivery date")
   elif photoLink!="" and DeliveryDate=="":
      cur.execute(f"""update Customization_Plan set Photo_links='{photoLink}' where Plan_ID={empPland[0][0]}""" )
      cur.commit()
      print("Successfully updated photo link") 
   elif photoLink!="" and DeliveryDate!="":
      cur.execute(f"""update Customization_Plan set Estimated_DeliveryDate='{DeliveryDate}',Photo_links='{photoLink}' where Plan_ID={empPland[0][0]}""" )
      cur.commit()
...
def Questions(empId):
   ...
   cur.execute(f"""select Question_No,Question_Desp from Questionnaire where Plan_ID={empPland[0][0]}""")
   result=[]
   for i in cur:
      result.append(i)
...
def UpdateQuestionsAns(empId,quesNo,quesAns):
   ...
   cur.execute(f"""update Questionnaire set Question_Ans='{quesAns}' where Question_No='{quesNo}' and Plan_ID={empPland[0][0]}""")
   cur.commit()
...
def DeleteVehicle(empId):
   ...
   cur.execute(f"""delete from Customization_Plan where Plan_ID={empPland[0][0]}""")
   cur.execute(f"""delete from Questionnaire where Plan_ID={empPland[0][0]}""")
   ...
   cur.execute(f"""delete from Customization_Detail where VerificationIN={vIn[0][0]}""")
